...
func TestContextColorThresholds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data := types.Payload{ContextWindow: types.ContextWindowInfo{UsedPercentage: new(tt.percent)}}
...
func TestFolderCurrentDirPriority(t *testing.T) {
	data := types.Payload{Cwd: "/fallback", Workspace: types.WorkspaceInfo{CurrentDir: "/preferred"}}
...
func TestPR(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			data: types.Payload{PR: &types.PRInfo{Number: 2, ReviewState: new("changes_requested")}},
...
func TestRateLimit(t *testing.T) {
	tests := []struct {
		...
		{name: "nil rate limits", data: types.Payload{}, want: ""},
		{
			name: "with five hour",
			data: types.Payload{
				RateLimits: &types.RateLimits{FiveHour: &types.RateLimitWindow{UsedPercentage: 23.5}},
			},
...
func TestToken(t *testing.T) {
	tests := []struct {
		...
		{name: "zero tokens", data: types.Payload{}, want: ""},
		{
			name: "with tokens",
			data: types.Payload{
				ContextWindow: types.ContextWindowInfo{TotalInputTokens: 15500, TotalOutputTokens: 1200},
			},
...
func TestCache(t *testing.T) {
	tests := []struct {
		...
		{name: "nil prompt cache", data: types.Payload{}, want: ""},
		{
			...
			data: types.Payload{PromptCache: &types.PromptCacheInfo{Warm: true, HitRatio: new(0.91)}},
